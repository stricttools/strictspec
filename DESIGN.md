...
- `go/` — THE TOOLCHAIN and the Go releasable: module `github.com/stricttools/strictspec/go`,
...
- Registries: PyPI `strictspec` (Python runtime + CLI stub), npm `strictspec` (TS runtime + CLI stub), Go module `github.com/stricttools/strictspec/go` (toolchain + Go runtime). One name per registry; there are no separate binary packages.
- Names confirmed by the owner 2026-07-27: PyPI `strictspec` is free; npm `strictspec` is owner-reserved (a 0.0.0 placeholder); GitHub `smm-h/strictspec` is to be created at scaffolding (transferred to `stricttools/strictspec` on 2026-09-20).
